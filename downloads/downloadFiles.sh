...
function DOWNLOAD_RHCOS {
	[[ -f rhcos-${RELEASE}.iso.done ]] || ( echo -n "Old file " && rm -fv rhcos.iso )
	...
	[[ -f rootfs-${RELEASE}.img.done ]] || ( echo -n "Old file " && rm -fv rootfs.img )
	...
	[[ -f kernel-${RELEASE}.done ]] || ( echo -n "Old file " && rm -fv kernel )
	...
	[[ -f initramfs-${RELEASE}.img.done ]] || ( echo -n "Old file " && rm -fv initramfs.img )
...
function DOWNLOAD_CLIENTS {
	[[ -f openshift-client-${RELEASE}.tar.gz.done ]] || ( echo -n "Old file " && rm -fv openshift-client.tar.gz )
	...
	[[ -f openshift-install-${RELEASE}.tar.gz.done ]] || ( echo -n "Old file " && rm -fv openshift-install.tar.gz )
