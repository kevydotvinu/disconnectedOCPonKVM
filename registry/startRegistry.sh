...
function RUN_REGISTRY {
	...
	mkdir -p /etc/docker/certs.d/localhost\:5000; cp ${DIR}/certs/server.pem /etc/docker/certs.d/localhost\:5000/ca.crt
	sleep 5s; curl -k -u openshift:redhat https://localhost:5000/v2/_catalog
	podman login --authfile ${DIR}/auth/auth.json -u openshift -p redhat 192.168.122.1:5000
